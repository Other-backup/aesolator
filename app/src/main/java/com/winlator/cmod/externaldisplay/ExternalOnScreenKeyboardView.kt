...
class ExternalOnScreenKeyboardView(
        ...
        keys.forEach { spec ->
            val button =
                Button(context).apply {
                    ...
                    setPadding(0, 0, 0, 0)
                    layoutParams = LayoutParams(0, height, spec.weight).apply { setMargins(margin, margin, margin, margin) }
    ...
    override fun onDetachedFromWindow() {
        downKeys.toList().forEach { key -> xServer.injectKeyRelease(key) }
        downKeys.clear()
    ...
    private fun createKeyBackground(
        ...
        fun shape(color: Int): GradientDrawable =
