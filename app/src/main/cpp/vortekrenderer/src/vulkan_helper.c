...
static bool containsString(ArrayList* list, const char* value) {
    if (!list || !value) return false;
    for (int i = 0; i < list->size; i++) {
        const char* item = list->elements[i];
        if (item && strcmp(item, value) == 0) return true;
    }
    return false;
}

static bool containsExtensionProperty(const VkExtensionProperties* properties, uint32_t propertyCount, const char* extensionName) {
    if (!properties || !extensionName) return false;
    for (uint32_t i = 0; i < propertyCount; i++) {
        if (strcmp(properties[i].extensionName, extensionName) == 0) return true;
    }
    return false;
}

static void appendDisabledDeviceExtensions(VkContext* context, const char* const* extensions, int extensionCount) {
    if (!context || !extensions || extensionCount <= 0) return;
    if (!context->disabledDeviceExtensions) context->disabledDeviceExtensions = calloc(1, sizeof(ArrayList));

    for (int i = 0; i < extensionCount; i++) {
        const char* extensionName = extensions[i];
        if (!extensionName || containsString(context->disabledDeviceExtensions, extensionName)) continue;
        ArrayList_add(context->disabledDeviceExtensions, strdup(extensionName));
    }
}

static uint32_t clampVkApiVersion(uint32_t first, uint32_t second) {
    if (first == 0) return second;
    if (second == 0) return first;
    return first <= second ? first : second;
}

static uint32_t queryHostVkApiVersion(void) {
    if (vulkanWrapper.vkEnumerateInstanceVersion) {
        uint32_t apiVersion = VK_API_VERSION_1_0;
        if (vulkanWrapper.vkEnumerateInstanceVersion(&apiVersion) == VK_SUCCESS) {
            return apiVersion;
        }
    }
...
uint32_t getEffectiveVkApiVersion(VkContext* context) {
    if (!context) return VK_API_VERSION_1_0;
    if (context->hostVkApiVersion == 0) {
        context->hostVkApiVersion = queryHostVkApiVersion();
    }

    uint32_t effectiveVersion = clampVkApiVersion((uint32_t) context->vkMaxVersion, context->hostVkApiVersion);
    if (context->physicalVkApiVersion != 0) {
        effectiveVersion = clampVkApiVersion(effectiveVersion, context->physicalVkApiVersion);
    }
    return effectiveVersion == 0 ? VK_API_VERSION_1_0 : effectiveVersion;
}
...
static void setupExposedDeviceExtensions(VkContext* context) {
    ...
    if (strcmp(context->engineName, "mesa zink") == 0) {
        ...
        appendDisabledDeviceExtensions(context, disabledDeviceExtensions, ARRAY_SIZE(disabledDeviceExtensions));
...
void initVulkanInstance(VkContext* context, VkInstance instance, const VkApplicationInfo* applicationInfo) {
    ...
    vulkanWrapper.vkGetPhysicalDeviceProperties(physicalDevices[0], &deviceProperties);
    context->physicalVkApiVersion = deviceProperties.apiVersion;
...
void injectRequiredDeviceExtensions(VkContext* context, VkPhysicalDevice physicalDevice, char*** extensions, uint32_t* extensionCount) {
    ...
    if (vulkanWrapper.vkEnumerateDeviceExtensionProperties(physicalDevice, NULL, &propertyCount, NULL) != VK_SUCCESS || propertyCount == 0) {
        return;
    }

    VkExtensionProperties* properties = vt_alloc(&context->memoryPool, propertyCount * sizeof(VkExtensionProperties));
    if (vulkanWrapper.vkEnumerateDeviceExtensionProperties(physicalDevice, NULL, &propertyCount, properties) != VK_SUCCESS) {
        return;
    }

    const char* extraExtensions[ARRAY_SIZE(requiredExtensions)];
    uint32_t extraExtensionCount = 0;
    for (int i = 0; i < ARRAY_SIZE(requiredExtensions); i++) {
        const char* extensionName = requiredExtensions[i];
        if (context->disabledDeviceExtensions && containsString(context->disabledDeviceExtensions, extensionName)) continue;
        if (context->exposedDeviceExtensions && !containsString(context->exposedDeviceExtensions, extensionName)) continue;
        if (!containsExtensionProperty(properties, propertyCount, extensionName)) continue;
        extraExtensions[extraExtensionCount++] = extensionName;
    }

    if (extraExtensionCount > 0) {
        injectExtensions(context, extensions, extensionCount, extraExtensions, extraExtensionCount, NULL, 0);
    }
}
...
void checkDeviceProperties(VkContext* context, VkPhysicalDeviceProperties* properties, void* pNext) {
    ...
    if (properties->apiVersion != 0) {
        context->physicalVkApiVersion = properties->apiVersion;
    }
    properties->apiVersion = getEffectiveVkApiVersion(context);
