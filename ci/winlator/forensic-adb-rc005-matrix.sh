...
set_global_fsr_mode() {
  ...
  local pref_rel="$3"
  local target tmp_xml seen
  local -a pref_targets pref_targets_uniq updated_targets

  pref_targets=(
    "${pref_rel}"
    "./shared_prefs/${WLT_PACKAGE}_preferences.xml"
    "./shared_prefs/com.winlator.cmod_preferences.xml"
  )
  pref_targets_uniq=()
  for target in "${pref_targets[@]}"; do
    [[ -n "${target}" ]] || continue
    seen=0
    for existing in "${pref_targets_uniq[@]}"; do
    ...
    if [[ "${seen}" == "0" ]]; then
      pref_targets_uniq+=("${target}")
    fi
  done

  updated_targets=()
  for target in "${pref_targets_uniq[@]}"; do
    tmp_xml="$(mktemp "${WLT_OUT_DIR}/.prefs.XXXXXX.xml")"
    adb_s shell "run-as ${WLT_PACKAGE} sh -c 'cat ${target}'" > "${tmp_xml}" 2>/dev/null || true
    if [[ ! -s "${tmp_xml}" ]]; then
      printf '<map/>\n' > "${tmp_xml}"
    fi
...
PY
    ...
    adb_s shell "run-as ${WLT_PACKAGE} sh -c 'mkdir -p ./shared_prefs && cat > ${target}'" < "${tmp_xml}"
    rm -f "${tmp_xml}"
    updated_targets+=("${target}")
  done

  if [[ "${#updated_targets[@]}" -gt 0 ]]; then
    log "fsr prefs updated mode=${fsr_mode} strength=${fsr_strength} targets=$(IFS=,; echo "${updated_targets[*]}")"
  fi
}
...
scenario_row_present() {
  local label="$1"
  [[ -f "${SCENARIO_INDEX_TSV}" ]] || return 1
  awk -F'\t' -v label="${label}" '
    NR == 1 { next }
    $1 == label { found = 1; exit }
    END { exit found ? 0 : 1 }
  ' "${SCENARIO_INDEX_TSV}"
}

append_scenario_row_once() {
  local label="$1"
  local flavor="$2"
  ...
  if scenario_row_present "${label}"; then
    return 0
  fi
...
scenario_is_complete() {
  local safe_label="$1"
  local scenario_dir="${WLT_OUT_DIR}/${safe_label}"
  [[ -d "${scenario_dir}" ]] || return 1
  [[ -f "${scenario_dir}/scenario_meta.txt" ]] || return 1
  [[ -f "${scenario_dir}/wait-status.txt" ]] || return 1
  [[ -f "${scenario_dir}/trace_id.txt" ]] || return 1
  return 0
}
...
capture_single_scenario() {
  ...
  if [[ "${WLT_RESUME}" == "1" ]] && scenario_is_complete "${safe_label}"; then
    append_scenario_row_once "${label}" "${flavor}" "${container_id}" "${vpn_state}" "${nvapi_state}" "${fsr_mode}"
    log "skip completed label=${label}"
    return 0
  fi
...
resolve_default_baseline() {
  local baseline
  baseline="$(awk -F'\t' '$2=="freewine11" && $4=="off" && $5=="0" && $6=="quality" {print $1; exit}' "${SCENARIO_INDEX_TSV}" || true)"
  if [[ -z "${baseline}" ]]; then
...
main() {
  ...
  local total expected index_header
  ...
  [[ "${WLT_RESUME}" =~ ^[01]$ ]] || fail "WLT_RESUME must be 0 or 1"
  ...
  index_header=$'label\tflavor\tcontainer_id\tvpn_state\tnvapi_requested\tproton_fsr_mode'
  if [[ "${WLT_RESUME}" == "1" && -f "${SCENARIO_INDEX_TSV}" ]]; then
    if ! head -n1 "${SCENARIO_INDEX_TSV}" | grep -qx "${index_header}"; then
      fail "existing matrix index has invalid header: ${SCENARIO_INDEX_TSV}"
    fi
    log "resume enabled: reusing ${SCENARIO_INDEX_TSV}"
  else
    printf '%s\n' "${index_header}" > "${SCENARIO_INDEX_TSV}"
  fi
