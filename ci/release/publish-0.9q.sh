...
WINLATOR_NOTES="${ROOT_DIR}/out/release-notes/winlator-v0.9q.md"
...
usage() {
  cat <<USAGE
Usage: bash ci/release/publish-0.9q.sh [--apply] [--app-repo owner/repo] [--wcp-repo owner/repo] [--winlator-tag TAG] [--wcp-tag TAG]
...
[[ -f "${WINLATOR_NOTES}" ]] || fail "Missing notes file: ${WINLATOR_NOTES} (run ci/release/prepare-0.9q-notes.sh)"
[[ -f "${WCP_NOTES}" ]] || fail "Missing notes file: ${WCP_NOTES} (run ci/release/prepare-0.9q-notes.sh)"
