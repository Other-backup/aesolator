...
#define LOG_TAG "AeroProcessLifecycle"
        ...
        for (;;) {
            ...
            if (reaped > 0) {
                __android_log_print(ANDROID_LOG_INFO, LOG_TAG,
                                    "Collected %d dead child processes", reaped);
            }

            struct timespec now = timespec_now();
            if (timespec_compare(now, deadline) >= 0) break;

            struct timespec sleep_until = timespec_add_ms(now, 100);
            if (timespec_compare(sleep_until, deadline) > 0) sleep_until = deadline;
...
Java_com_winlator_cmod_core_ProcessHelper_nativeReapDeadChildrenNow(
...
Java_com_winlator_cmod_core_ProcessHelper_nativeStartNativeReaperWindow(
    ...
    struct timespec new_deadline = timespec_add_ms(timespec_now(), duration_ms);
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    /*
     * Java Process.waitFor() owns direct children. A process-wide SIGCHLD
     * handler would race it, so cleanup is explicit and windowed.
     */
    int reaped = reap_dead_children_now();
    if (reaped > 0) {
        __android_log_print(ANDROID_LOG_INFO, LOG_TAG,
