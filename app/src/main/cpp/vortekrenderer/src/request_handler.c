...
static bool vt_canFallbackAttachmentReference2(const VkAttachmentReference2* reference) {
    return !reference || (reference->pNext == NULL && reference->aspectMask == 0);
}

static VkAttachmentReference* vt_convertAttachmentReferences(MemoryPool* memoryPool, const VkAttachmentReference2* references, uint32_t count) {
    if (!references || count == 0) return NULL;

    VkAttachmentReference* converted = vt_alloc(memoryPool, count * sizeof(VkAttachmentReference));
    for (uint32_t i = 0; i < count; i++) {
        converted[i].attachment = references[i].attachment;
        converted[i].layout = references[i].layout;
    }
    return converted;
}

static bool vt_canFallbackRenderPassCreateInfo2(const VkRenderPassCreateInfo2* createInfo) {
    if (!createInfo || createInfo->pNext != NULL) return false;
    if (createInfo->correlatedViewMaskCount != 0 || createInfo->pCorrelatedViewMasks != NULL) return false;

    for (uint32_t i = 0; i < createInfo->attachmentCount; i++) {
        if (createInfo->pAttachments[i].pNext != NULL) return false;
    }

    for (uint32_t i = 0; i < createInfo->subpassCount; i++) {
        const VkSubpassDescription2* subpass = &createInfo->pSubpasses[i];
        if (subpass->pNext != NULL || subpass->viewMask != 0) return false;

        for (uint32_t j = 0; j < subpass->inputAttachmentCount; j++) {
            if (!vt_canFallbackAttachmentReference2(&subpass->pInputAttachments[j])) return false;
        }
        for (uint32_t j = 0; j < subpass->colorAttachmentCount; j++) {
            if (!vt_canFallbackAttachmentReference2(&subpass->pColorAttachments[j])) return false;
        }
        if (subpass->pResolveAttachments) {
            for (uint32_t j = 0; j < subpass->colorAttachmentCount; j++) {
                if (!vt_canFallbackAttachmentReference2(&subpass->pResolveAttachments[j])) return false;
            }
        }
        if (!vt_canFallbackAttachmentReference2(subpass->pDepthStencilAttachment)) return false;
    }

    for (uint32_t i = 0; i < createInfo->dependencyCount; i++) {
        const VkSubpassDependency2* dependency = &createInfo->pDependencies[i];
        if (dependency->pNext != NULL || dependency->viewOffset != 0) return false;
    }

    return true;
}

static VkResult vt_createRenderPassLegacy(VkContext* context, VkDevice device, const VkRenderPassCreateInfo2* createInfo, VkRenderPass* renderPass) {
    if (!vulkanWrapper.vkCreateRenderPass || !vt_canFallbackRenderPassCreateInfo2(createInfo)) {
        return VK_ERROR_EXTENSION_NOT_PRESENT;
    }

    VkAttachmentDescription* attachments = NULL;
    if (createInfo->attachmentCount > 0) {
        attachments = vt_alloc(&context->memoryPool, createInfo->attachmentCount * sizeof(VkAttachmentDescription));
        for (uint32_t i = 0; i < createInfo->attachmentCount; i++) {
            const VkAttachmentDescription2* src = &createInfo->pAttachments[i];
    ...
    if (createInfo->subpassCount > 0) {
        subpasses = vt_alloc(&context->memoryPool, createInfo->subpassCount * sizeof(VkSubpassDescription));
        for (uint32_t i = 0; i < createInfo->subpassCount; i++) {
            const VkSubpassDescription2* src = &createInfo->pSubpasses[i];
            VkSubpassDescription* dst = &subpasses[i];
            dst->flags = src->flags;
            dst->pipelineBindPoint = src->pipelineBindPoint;
            dst->inputAttachmentCount = src->inputAttachmentCount;
            dst->pInputAttachments = vt_convertAttachmentReferences(&context->memoryPool, src->pInputAttachments, src->inputAttachmentCount);
            dst->colorAttachmentCount = src->colorAttachmentCount;
            dst->pColorAttachments = vt_convertAttachmentReferences(&context->memoryPool, src->pColorAttachments, src->colorAttachmentCount);
            dst->pResolveAttachments = vt_convertAttachmentReferences(&context->memoryPool, src->pResolveAttachments, src->colorAttachmentCount);
            dst->pDepthStencilAttachment = vt_convertAttachmentReferences(&context->memoryPool, src->pDepthStencilAttachment, src->pDepthStencilAttachment ? 1 : 0);
    ...
    if (createInfo->dependencyCount > 0) {
        dependencies = vt_alloc(&context->memoryPool, createInfo->dependencyCount * sizeof(VkSubpassDependency));
        for (uint32_t i = 0; i < createInfo->dependencyCount; i++) {
            const VkSubpassDependency2* src = &createInfo->pDependencies[i];
            VkSubpassDependency* dst = &dependencies[i];
    ...
    VkRenderPassCreateInfo legacyCreateInfo = {0};
    legacyCreateInfo.sType = VK_STRUCTURE_TYPE_RENDER_PASS_CREATE_INFO;
    legacyCreateInfo.flags = createInfo->flags;
    legacyCreateInfo.attachmentCount = createInfo->attachmentCount;
    legacyCreateInfo.pAttachments = attachments;
    legacyCreateInfo.subpassCount = createInfo->subpassCount;
    legacyCreateInfo.pSubpasses = subpasses;
    legacyCreateInfo.dependencyCount = createInfo->dependencyCount;
    legacyCreateInfo.pDependencies = dependencies;

    return vulkanWrapper.vkCreateRenderPass(device, &legacyCreateInfo, NULL, renderPass);
}

static VkSubpassContents vt_getLegacySubpassContents(const VkSubpassBeginInfo* subpassBeginInfo) {
    return subpassBeginInfo ? subpassBeginInfo->contents : VK_SUBPASS_CONTENTS_INLINE;
}

static bool vt_isTrackedHandle(VkContext* context, VkObjectType type, void* handle) {
    return handle != VK_NULL_HANDLE && isTrackedVkHandle(context, type, handle);
}

static bool vt_isTrackedOptionalHandle(VkContext* context, VkObjectType type, void* handle) {
    return handle == VK_NULL_HANDLE || isTrackedVkHandle(context, type, handle);
}

static bool vt_descriptorTypeUsesImageInfo(VkDescriptorType descriptorType) {
...
static bool vt_descriptorTypeUsesBufferInfo(VkDescriptorType descriptorType) {
    switch (descriptorType) {
...
static bool vt_descriptorTypeUsesTexelBufferView(VkDescriptorType descriptorType) {
    switch (descriptorType) {
...
static bool vt_sanitizeDescriptorImageInfos(VkContext* context, VkWriteDescriptorSet* descriptorWrite) {
    if (!descriptorWrite->pImageInfo) return false;
    if (!vt_isTrackedHandle(context, VK_OBJECT_TYPE_DESCRIPTOR_SET, descriptorWrite->dstSet)) return false;

    for (uint32_t i = 0; i < descriptorWrite->descriptorCount; i++) {
        const VkDescriptorImageInfo* imageInfo = &descriptorWrite->pImageInfo[i];
        switch (descriptorWrite->descriptorType) {
            case VK_DESCRIPTOR_TYPE_SAMPLER:
                if (!vt_isTrackedOptionalHandle(context, VK_OBJECT_TYPE_SAMPLER, imageInfo->sampler)) return false;
                break;
            case VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER:
                if (!vt_isTrackedHandle(context, VK_OBJECT_TYPE_IMAGE_VIEW, imageInfo->imageView)) return false;
                if (!vt_isTrackedOptionalHandle(context, VK_OBJECT_TYPE_SAMPLER, imageInfo->sampler)) return false;
                break;
            case VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE:
            case VK_DESCRIPTOR_TYPE_STORAGE_IMAGE:
...
static bool vt_sanitizeDescriptorBufferInfos(VkContext* context, VkWriteDescriptorSet* descriptorWrite) {
    if (!descriptorWrite->pBufferInfo) return false;
    if (!vt_isTrackedHandle(context, VK_OBJECT_TYPE_DESCRIPTOR_SET, descriptorWrite->dstSet)) return false;

    for (uint32_t i = 0; i < descriptorWrite->descriptorCount; i++) {
        if (!vt_isTrackedHandle(context, VK_OBJECT_TYPE_BUFFER, descriptorWrite->pBufferInfo[i].buffer)) return false;
    }
...
static bool vt_sanitizeDescriptorTexelBufferViews(VkContext* context, VkWriteDescriptorSet* descriptorWrite) {
    if (!descriptorWrite->pTexelBufferView) return false;
    if (!vt_isTrackedHandle(context, VK_OBJECT_TYPE_DESCRIPTOR_SET, descriptorWrite->dstSet)) return false;

    for (uint32_t i = 0; i < descriptorWrite->descriptorCount; i++) {
        if (!vt_isTrackedHandle(context, VK_OBJECT_TYPE_BUFFER_VIEW, descriptorWrite->pTexelBufferView[i])) return false;
    }
...
static bool vt_sanitizeDescriptorWrite(VkContext* context, VkWriteDescriptorSet* descriptorWrite) {
    if (!descriptorWrite || descriptorWrite->dstSet == VK_NULL_HANDLE || descriptorWrite->descriptorCount == 0) return false;
    if (!vt_isTrackedHandle(context, VK_OBJECT_TYPE_DESCRIPTOR_SET, descriptorWrite->dstSet)) return false;

    if (vt_descriptorTypeUsesImageInfo(descriptorWrite->descriptorType)) {
        return vt_sanitizeDescriptorImageInfos(context, descriptorWrite);
    }

    if (vt_descriptorTypeUsesBufferInfo(descriptorWrite->descriptorType)) {
        return vt_sanitizeDescriptorBufferInfos(context, descriptorWrite);
    }

    if (vt_descriptorTypeUsesTexelBufferView(descriptorWrite->descriptorType)) {
        return vt_sanitizeDescriptorTexelBufferViews(context, descriptorWrite);
    }

#ifdef VK_DESCRIPTOR_TYPE_INLINE_UNIFORM_BLOCK
    if (descriptorWrite->descriptorType == VK_DESCRIPTOR_TYPE_INLINE_UNIFORM_BLOCK) {
        VkWriteDescriptorSetInlineUniformBlock* inlineUniformBlock =
                findNextVkStructure((void*) descriptorWrite->pNext, VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET_INLINE_UNIFORM_BLOCK);
        descriptorWrite->pImageInfo = NULL;
        descriptorWrite->pBufferInfo = NULL;
        descriptorWrite->pTexelBufferView = NULL;
        return inlineUniformBlock && inlineUniformBlock->pData && inlineUniformBlock->dataSize >= descriptorWrite->descriptorCount;
    }
#endif
...
static uint32_t vt_compactDescriptorWrites(VkContext* context, VkWriteDescriptorSet* descriptorWrites, uint32_t descriptorWriteCount) {
    uint32_t sanitizedCount = 0;
    for (uint32_t i = 0; i < descriptorWriteCount; i++) {
        VkWriteDescriptorSet descriptorWrite = descriptorWrites[i];
        if (!vt_sanitizeDescriptorWrite(context, &descriptorWrite)) continue;
        descriptorWrites[sanitizedCount++] = descriptorWrite;
    }
    return sanitizedCount;
}

static uint32_t vt_compactDescriptorCopies(VkContext* context, VkCopyDescriptorSet* descriptorCopies, uint32_t descriptorCopyCount) {
    uint32_t sanitizedCount = 0;
    for (uint32_t i = 0; i < descriptorCopyCount; i++) {
        VkCopyDescriptorSet descriptorCopy = descriptorCopies[i];
        if (descriptorCopy.srcSet == VK_NULL_HANDLE || descriptorCopy.dstSet == VK_NULL_HANDLE || descriptorCopy.descriptorCount == 0) continue;
        if (!vt_isTrackedHandle(context, VK_OBJECT_TYPE_DESCRIPTOR_SET, descriptorCopy.srcSet)) continue;
        if (!vt_isTrackedHandle(context, VK_OBJECT_TYPE_DESCRIPTOR_SET, descriptorCopy.dstSet)) continue;
        descriptorCopies[sanitizedCount++] = descriptorCopy;
    }
    return sanitizedCount;
}

static uint32_t vt_compactDescriptorSets(VkContext* context, VkDescriptorSet* descriptorSets, uint32_t descriptorSetCount) {
    uint32_t validCount = 0;
    for (uint32_t i = 0; i < descriptorSetCount; i++) {
        if (!vt_isTrackedHandle(context, VK_OBJECT_TYPE_DESCRIPTOR_SET, descriptorSets[i])) continue;
        descriptorSets[validCount++] = descriptorSets[i];
    }
    return validCount;
}

static uint32_t vt_compactCommandBuffers(VkContext* context, VkCommandBuffer* commandBuffers, uint32_t commandBufferCount) {
    uint32_t validCount = 0;
    for (uint32_t i = 0; i < commandBufferCount; i++) {
        if (!vt_isTrackedHandle(context, VK_OBJECT_TYPE_COMMAND_BUFFER, commandBuffers[i])) continue;
        commandBuffers[validCount++] = commandBuffers[i];
    }
    return validCount;
}
...
void vt_handle_vkCreateDevice(VkContext* context) {
    ...
    injectRequiredDeviceExtensions(context, physicalDevice, (char***)&createInfo.ppEnabledExtensionNames, &createInfo.enabledExtensionCount);
...
void vt_handle_vkEnumerateInstanceVersion(VkContext* context) {
    uint32_t effectiveApiVersion = getEffectiveVkApiVersion(context);
    vt_send(context->clientRing, VK_SUCCESS, &effectiveApiVersion, sizeof(effectiveApiVersion));
}
...
void vt_handle_vkCreateBuffer(VkContext* context) {
    ...
    if (result == VK_SUCCESS && buffer != VK_NULL_HANDLE) trackVkHandle(context, VK_OBJECT_TYPE_BUFFER, buffer, NULL);
...
void vt_handle_vkDestroyBuffer(VkContext* context) {
    ...
    VkDevice device = VkObject_fromId(deviceId);
    VkBuffer buffer = VkObject_fromId(bufferId);
    if (!vt_isTrackedHandle(context, VK_OBJECT_TYPE_BUFFER, buffer)) return;
    ...
    untrackVkHandle(context, VK_OBJECT_TYPE_BUFFER, buffer);
}
...
void vt_handle_vkCreateBufferView(VkContext* context) {
    ...
    if (result == VK_SUCCESS && view != VK_NULL_HANDLE) trackVkHandle(context, VK_OBJECT_TYPE_BUFFER_VIEW, view, NULL);
...
void vt_handle_vkDestroyBufferView(VkContext* context) {
    ...
    if (!vt_isTrackedHandle(context, VK_OBJECT_TYPE_BUFFER_VIEW, bufferView)) return;

    vulkanWrapper.vkDestroyBufferView(device, bufferView, NULL);
    untrackVkHandle(context, VK_OBJECT_TYPE_BUFFER_VIEW, bufferView);
}
...
void vt_handle_vkCreateImageView(VkContext* context) {
    ...
    if (result == VK_SUCCESS && view != VK_NULL_HANDLE) trackVkHandle(context, VK_OBJECT_TYPE_IMAGE_VIEW, view, NULL);
...
void vt_handle_vkDestroyImageView(VkContext* context) {
    ...
    if (!vt_isTrackedHandle(context, VK_OBJECT_TYPE_IMAGE_VIEW, imageView)) return;

    vulkanWrapper.vkDestroyImageView(device, imageView, NULL);
    untrackVkHandle(context, VK_OBJECT_TYPE_IMAGE_VIEW, imageView);
}
...
void vt_handle_vkCreatePipelineLayout(VkContext* context) {
    ...
    if (result == VK_SUCCESS && pipelineLayout != VK_NULL_HANDLE) trackVkHandle(context, VK_OBJECT_TYPE_PIPELINE_LAYOUT, pipelineLayout, NULL);
...
void vt_handle_vkDestroyPipelineLayout(VkContext* context) {
    ...
    if (!vt_isTrackedHandle(context, VK_OBJECT_TYPE_PIPELINE_LAYOUT, pipelineLayout)) return;

    vulkanWrapper.vkDestroyPipelineLayout(device, pipelineLayout, NULL);
    untrackVkHandle(context, VK_OBJECT_TYPE_PIPELINE_LAYOUT, pipelineLayout);
}
...
void vt_handle_vkCreateSampler(VkContext* context) {
    ...
    if (result == VK_SUCCESS && sampler != VK_NULL_HANDLE) trackVkHandle(context, VK_OBJECT_TYPE_SAMPLER, sampler, NULL);
...
void vt_handle_vkDestroySampler(VkContext* context) {
    ...
    if (!vt_isTrackedHandle(context, VK_OBJECT_TYPE_SAMPLER, sampler)) return;

    vulkanWrapper.vkDestroySampler(device, sampler, NULL);
    untrackVkHandle(context, VK_OBJECT_TYPE_SAMPLER, sampler);
}
...
void vt_handle_vkCreateDescriptorSetLayout(VkContext* context) {
    ...
    if (result == VK_SUCCESS && setLayout != VK_NULL_HANDLE) trackVkHandle(context, VK_OBJECT_TYPE_DESCRIPTOR_SET_LAYOUT, setLayout, NULL);
...
void vt_handle_vkDestroyDescriptorSetLayout(VkContext* context) {
    ...
    if (!vt_isTrackedHandle(context, VK_OBJECT_TYPE_DESCRIPTOR_SET_LAYOUT, descriptorSetLayout)) return;

    vulkanWrapper.vkDestroyDescriptorSetLayout(device, descriptorSetLayout, NULL);
    untrackVkHandle(context, VK_OBJECT_TYPE_DESCRIPTOR_SET_LAYOUT, descriptorSetLayout);
}
...
void vt_handle_vkCreateDescriptorPool(VkContext* context) {
    ...
    if (result == VK_SUCCESS && descriptorPool != VK_NULL_HANDLE) trackVkHandle(context, VK_OBJECT_TYPE_DESCRIPTOR_POOL, descriptorPool, NULL);
...
void vt_handle_vkDestroyDescriptorPool(VkContext* context) {
    ...
    if (!vt_isTrackedHandle(context, VK_OBJECT_TYPE_DESCRIPTOR_POOL, descriptorPool)) return;

    vulkanWrapper.vkDestroyDescriptorPool(device, descriptorPool, NULL);
    untrackVkHandlesByOwner(context, VK_OBJECT_TYPE_DESCRIPTOR_SET, descriptorPool);
    untrackVkHandle(context, VK_OBJECT_TYPE_DESCRIPTOR_POOL, descriptorPool);
}
...
void vt_handle_vkResetDescriptorPool(VkContext* context) {
    ...
    VkResult result = vulkanWrapper.vkResetDescriptorPool(device, descriptorPool, flags);
    if (result == VK_SUCCESS) untrackVkHandlesByOwner(context, VK_OBJECT_TYPE_DESCRIPTOR_SET, descriptorPool);
}
...
void vt_handle_vkAllocateDescriptorSets(VkContext* context) {
    ...
    if (result == VK_SUCCESS) {
        for (uint32_t i = 0; i < allocateInfo.descriptorSetCount; i++) {
            if (descriptorSets[i] != VK_NULL_HANDLE) trackVkHandle(context, VK_OBJECT_TYPE_DESCRIPTOR_SET, descriptorSets[i], allocateInfo.descriptorPool);
        }
    }
...
void vt_handle_vkFreeDescriptorSets(VkContext* context) {
    ...
    descriptorSetCount = vt_compactDescriptorSets(context, descriptorSets, descriptorSetCount);
    if (descriptorSetCount == 0) return;

    VkResult result = vulkanWrapper.vkFreeDescriptorSets(device, descriptorPool, descriptorSetCount, descriptorSets);
    if (result == VK_SUCCESS) {
        for (uint32_t i = 0; i < descriptorSetCount; i++) untrackVkHandle(context, VK_OBJECT_TYPE_DESCRIPTOR_SET, descriptorSets[i]);
    }
}
...
void vt_handle_vkUpdateDescriptorSets(VkContext* context) {
    ...
    descriptorWriteCount = vt_compactDescriptorWrites(context, descriptorWrites, descriptorWriteCount);
    descriptorCopyCount = vt_compactDescriptorCopies(context, descriptorCopies, descriptorCopyCount);
...
void vt_handle_vkCreateCommandPool(VkContext* context) {
    ...
    if (result == VK_SUCCESS && commandPool != VK_NULL_HANDLE) trackVkHandle(context, VK_OBJECT_TYPE_COMMAND_POOL, commandPool, NULL);
...
void vt_handle_vkDestroyCommandPool(VkContext* context) {
    ...
    if (!vt_isTrackedHandle(context, VK_OBJECT_TYPE_COMMAND_POOL, commandPool)) return;

    vulkanWrapper.vkDestroyCommandPool(device, commandPool, NULL);
    untrackVkHandlesByOwner(context, VK_OBJECT_TYPE_COMMAND_BUFFER, commandPool);
    untrackVkHandle(context, VK_OBJECT_TYPE_COMMAND_POOL, commandPool);
}
...
void vt_handle_vkResetCommandPool(VkContext* context) {
    ...
    VkResult result = vulkanWrapper.vkResetCommandPool(device, commandPool, flags);
    if (result == VK_SUCCESS) untrackVkHandlesByOwner(context, VK_OBJECT_TYPE_COMMAND_BUFFER, commandPool);
}
...
void vt_handle_vkAllocateCommandBuffers(VkContext* context) {
    ...
    if (result == VK_SUCCESS) {
        for (uint32_t i = 0; i < allocateInfo.commandBufferCount; i++) {
            if (commandBuffers[i] != VK_NULL_HANDLE) trackVkHandle(context, VK_OBJECT_TYPE_COMMAND_BUFFER, commandBuffers[i], allocateInfo.commandPool);
        }
    }
...
void vt_handle_vkFreeCommandBuffers(VkContext* context) {
    ...
    commandBufferCount = vt_compactCommandBuffers(context, commandBuffers, commandBufferCount);
    if (commandBufferCount == 0) return;

    vulkanWrapper.vkFreeCommandBuffers(device, commandPool, commandBufferCount, commandBuffers);
    for (uint32_t i = 0; i < commandBufferCount; i++) untrackVkHandle(context, VK_OBJECT_TYPE_COMMAND_BUFFER, commandBuffers[i]);
}
...
void vt_handle_vkCmdPushDescriptorSetKHR(VkContext* context) {
    ...
    VkCommandBuffer commandBuffer = VkObject_fromId(commandBufferId);
    VkPipelineLayout layout = VkObject_fromId(layoutId);
    if (!vt_isTrackedHandle(context, VK_OBJECT_TYPE_COMMAND_BUFFER, commandBuffer)) return;
    if (!vt_isTrackedHandle(context, VK_OBJECT_TYPE_PIPELINE_LAYOUT, layout)) return;
    ...
    descriptorWriteCount = vt_compactDescriptorWrites(context, descriptorWrites, descriptorWriteCount);
    if (descriptorWriteCount == 0) return;
...
void vt_handle_vkCreateRenderPass2(VkContext* context) {
    ...
    VkResult result = vulkanWrapper.vkCreateRenderPass2
            ? vulkanWrapper.vkCreateRenderPass2(device, &createInfo, NULL, &renderPass)
            : vt_createRenderPassLegacy(context, device, &createInfo, &renderPass);
...
void vt_handle_vkCmdBeginRenderPass2(VkContext* context) {
    ...
    if (vulkanWrapper.vkCmdBeginRenderPass2) {
        vulkanWrapper.vkCmdBeginRenderPass2(commandBuffer, &renderPassBegin, &subpassBeginInfo);
    }
    else if (vulkanWrapper.vkCmdBeginRenderPass) {
        vulkanWrapper.vkCmdBeginRenderPass(commandBuffer, &renderPassBegin, vt_getLegacySubpassContents(&subpassBeginInfo));
    }
}
...
void vt_handle_vkCmdNextSubpass2(VkContext* context) {
    ...
    if (vulkanWrapper.vkCmdNextSubpass2) {
        vulkanWrapper.vkCmdNextSubpass2(commandBuffer, &subpassBeginInfo, &subpassEndInfo);
    }
    else if (vulkanWrapper.vkCmdNextSubpass) {
        vulkanWrapper.vkCmdNextSubpass(commandBuffer, vt_getLegacySubpassContents(&subpassBeginInfo));
    }
}
...
void vt_handle_vkCmdEndRenderPass2(VkContext* context) {
    ...
    if (vulkanWrapper.vkCmdEndRenderPass2) {
        vulkanWrapper.vkCmdEndRenderPass2(commandBuffer, &subpassEndInfo);
    }
    else if (vulkanWrapper.vkCmdEndRenderPass) {
        vulkanWrapper.vkCmdEndRenderPass(commandBuffer);
    }
}
