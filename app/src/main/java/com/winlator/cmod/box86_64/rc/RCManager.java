...
public class RCManager {
    ...
    private void copyAssetRCFilesIfNeeded() {
        File rcfilesDir = getRCFilesDir(context);
    ...
    public void loadRCFiles() {
        ...
        LinkedList<RCFile> loaded = new LinkedList<>();
        ...
        try {
            ...
            for (int i = 0; i < groupsJSONArray.length(); i++) {
                ...
                for (int j = 0; j < itemsJSONArray.length(); j++) {
                    ...
                    items.add(new RCItem(processName, itemDesc, map));
                }
                groups.add(new RCGroup(groupName, groupDesc, groupEnabled, items));
            }

            RCFile rcfile = new RCFile(context, rcfileId);
            rcfile.setName(rcfileName);
            for (RCGroup group : groups) rcfile.getGroups().add(group);
            return rcfile;
        } catch (JSONException e) {
            e.printStackTrace();
        }
    ...
    public void saveAllRCFiles() {
        if (rcfiles != null) {
            for (RCFile rcfile : rcfiles) rcfile.save();
        }
    }
    ...
    public static void loadRCFileSpinner(RCManager rcManager, int rcfileId, Spinner spinner, Callback<Integer> callback) {
        ...
        fileNames.add("-- " + context.getString(R.string.disabled) + " --");
        for (RCFile rcfile : rcFiles) fileNames.add(rcfile.getName());

        spinner.setAdapter(new ArrayAdapter<>(context, android.R.layout.simple_spinner_dropdown_item, fileNames));
