...
def audit_groups(
    ...
    # In a fully folded one-patch baseline, some helper files may already exist in the
    # tracked Winlator checkout and therefore not appear as separate added hunks in
    # 0001. Fall back to the local source baseline so the audit still validates the
    # effective consolidated state instead of only diff visibility.
    if patch_count == 1:
        mainline_name = next(iter(sorted(patch_dir.glob("*.patch")))).name
        repo_root = patch_dir.parents[2]
        source_root = repo_root / "work" / "winlator-ludashi" / "src"
        for group in GROUP_RULES:
            if grouped_lines.get(group.name):
                continue
            for alias in group.aliases:
                source_path = source_root / alias
                if not source_path.is_file():
                    continue
                baseline_lines = source_path.read_text(encoding="utf-8", errors="ignore").splitlines()
                grouped_lines[group.name].extend((mainline_name, line) for line in baseline_lines)
                break
