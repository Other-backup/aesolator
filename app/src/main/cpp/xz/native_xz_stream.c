...
Java_com_winlator_cmod_core_NativeXzInputStream_nativeOpen(JNIEnv *env,
                                                           jclass clazz,
                                                           jstring path) {
  (void)clazz;
  ...
  if (stream->file == NULL) {
    ...
    snprintf(error_message, sizeof(error_message),
             "Failed to open XZ source: %s", strerror(errno));
...
Java_com_winlator_cmod_core_NativeXzInputStream_nativeRead(
    JNIEnv *env, jclass clazz, jlong handle, jbyteArray output, jint offset,
    jint length) {
  (void)clazz;
  ...
  if (output == NULL || offset < 0 || length < 0 ||
      offset > (*env)->GetArrayLength(env, output) - length) {
    throw_io_exception(env, "Invalid native XZ read buffer range");
    return -1;
  }
  ...
  while (stream->buffer.out_pos < stream->buffer.out_size) {
    ...
    if (stream->buffer.in_pos == stream->buffer.in_size &&
        !stream->input_finished) {
      size_t amount_read = fread(stream->input_buffer, 1,
                                 sizeof(stream->input_buffer), stream->file);
      if (amount_read == 0) {
    ...
    enum xz_ret ret = xz_dec_catrun(stream->decoder, &stream->buffer,
                                    stream->input_finished ? 1 : 0);
    ...
    if (ret == XZ_OK) {
      ...
      if (stream->buffer.in_pos == input_before &&
          stream->buffer.out_pos == output_before) {
...
Java_com_winlator_cmod_core_NativeXzInputStream_nativeClose(JNIEnv *env,
                                                            jclass clazz,
                                                            jlong handle) {
  (void)env;
  (void)clazz;
